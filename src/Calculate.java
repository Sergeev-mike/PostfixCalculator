...
public class Calculate {
    ...
    /**
     * @param str строка с выражением для подсчёта
     * @return результат строкой (String)
    */
    public String start(String str) {
        String nu = this.convertInfixToPostfix(str);
        String res = this.calculatePostfixExpression(nu);
        return res;
    }
    ...
    private static String gatherNumber(char ch, String str, int index) {
        ...
        while (Character.isDigit(ch) || ch == ',' || ch == '.' || ch == '±') {
            sb.append(ch);
            index++;
            if (index >= strLength) break; //следим за выходом за границы строки
            ch = str.charAt(index);
        }
     ...
     * некорректно.
     */
    private static String convertInfixToPostfix(String infixStr) {
     ...
     * некорректно.
     */
    private static String calculatePostfixExpression(String postfixStr) {
